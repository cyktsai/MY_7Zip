...
bool SplitCommandLine(const UString &src, UString &dest1, UString &dest2)
{
  ...
  {
    wchar_t c = src[i];
    if ((c == L' ' || c == L'\t') && !quoteMode)//interval blank and not in the quoteMode ("C:\tmp.txt")
    {
      dest2 = src.Ptr(i + 1); //cut the string from the blank and assign the left to dest2
      return i != 0; //once break back to the caller function the dest2 will be copied to sTemp
    }
    if (c == L'\"')
      quoteMode = !quoteMode;
    else
...
void SplitCommandLine(const UString &s, UStringVector &parts)
{
  ...
  parts.Clear(); // to clear the commandString first
  for (;;)
  {
    UString s1, s2;
